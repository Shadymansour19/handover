-- Handover — proposed Supabase (Postgres) schema + RLS
--
-- STATUS: proposal / reference only — not yet applied to any Supabase project.
-- See SPEC.md for the decisions and assumptions behind these choices.
--
-- Run this in the Supabase SQL editor (or via `supabase db push` with a
-- migration file) against a fresh project. Idempotent-ish via IF NOT EXISTS
-- / CREATE OR REPLACE where practical, but treat as a one-time initial
-- migration, not a repeatable script.
