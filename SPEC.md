...
| Can an admin delete a user account? | **Yes — hard delete, Edge Function action `delete`** | Same `service_role` requirement (`auth.admin.deleteUser`). `profiles.id` cascades on delete, but `maintenance_records.created_by`/`operation_events.created_by` do **not** (plain `REFERENCES`, no `ON DELETE` clause) — deleting a user who has ever created a record fails with a foreign-key error instead of silently orphaning/erasing their audit trail. Deliberate: such a user needs deactivating (`is_active = false`), not deleting. Blocked for deleting your own account, same self-lockout reasoning as demote/deactivate. |
